...
//  printf("!!!!%s\n", my_fn);
  ...
  //printf("\n\n\n\n\n\n\n\n\nfilename ::: !!!! %s\n", my_fn);
  //sema_init(&ld_ex_sema,1);
   if(ld_ex_sema == NULL) 
 {
//		 printf("\n\n\n\n\n\n\ninit\n");
		 ld_ex_sema = (struct semaphore * )malloc(sizeof(struct semaphore));
		 sema_init(ld_ex_sema,1);
 }
...
//	printf("down!!\n");
 ...
 //	printf("start exec\n");
   ...
   file = filesys_open(my_fn);
	sema_up(ld_ex_sema);
  if(!file) tid = TID_ERROR;
  else 
  {
		 ...
		 // printf("here pro exe\n");
		  tid = thread_create (my_fn, PRI_DEFAULT, start_process, fn_copy);
		  if(tid != TID_ERROR)
		  {
				  getStruct(tchild, thread_current()->children, tid, struct thread, sibling_elem, tid);
				  tchild->exefile = file;
		  }
	//여기
		  //	  if(exelist)
	//			  list_init();
	//	  list_push_back(exelist, );
 ...
 while(i-- > 0){
  ...
  return -1;*/
	//find this sema
		...
		tchild->is_par_waiting = true;
		if(tchild->iszombi)
		{
			//	currTh->iswaiting = false;
				sema_up(&(tchild->zombisema));
				tchild->iszombi = false;
		}

	//	currTh->iswaiting = true;
...
process_exit (void)
{
  ...
  int i;
	struct list_elem * e;	
	struct myfile  * file;	
	struct list l = thread_current()->filelist;
...
/*  if(!(cur->parent->iswaiting))
  ...
  ///////yj
//p/rintf("im %x and my par %x.\n", cur->tid, cur->parent->tid);
//  sema_up(&(cur->parent->smph));
//printf("?????name %s.\n", thread_current()->name);

  file_close(thread_current()->exefile);
  intr_disable();
...
/*	struct list_elem * e;	
	...
	{	
			file = list_entry(e, struct myfile ,fe); 	
		if(file->fd > 1)
		{
	//		printf("디져라 파일년 %d\n", file->fd);
			file_close(file->fp);
...
//	printf("  블록 머냐\n");
 ...
 // getStruct(thread_current()->filelist, );
...
/////////////////////////////must modify
			  ...
			  {
					  int temp;
					  temp = ( read_bytes + zero_bytes ) / PGSIZE;
					  t->numofpages = put(temp);
			  }
			#endif
			  /////////////////////////////////////
...
//		printf("addr!!\n");
...

		 {
			unsigned int addr = *esp;
			{
				...
				//	 printf("%x\n", rem);
		 			 for(i = 0 ; i<sizeof(addr) ; i++)
		 			 {
						if(*esp-1<0){ success = false; goto done;}
						memset(--(*esp),rem/(16777216),1 );
				//		printf("%x %d", rem, rem);
						rem %= 16777216;
						rem *= (16*16);
		  ...
		  *esp -= sizeof(void*);
		 if(*esp<0){ success = false; goto done;}
